...
启动本地 Web 控制台（Flask JSON API + Vue 3 SPA）：

```bash
# 1. 构建前端（仅第一次或前端代码变更后）
rtk npm --prefix frontend run build

# 2. 启动 Flask 后端（默认端口 8765）
rtk .venv/bin/python scripts/task_console_web.py --host 127.0.0.1 --port 8765
```

**开发模式**（Vite 热重载，推荐前端联调时使用）：
...
打开终端输出的 URL 后，浏览器加载 Vue 3 SPA（**Tailwind + Material Symbols**）三栏 cockpit：左侧按会话分组任务，中间为对话与追加指令，右侧为详情与动作（interrupt/ack/retry）；可用 `/?task=<task_id>` 直达某任务，`?q=` 搜索过滤。

**架构**：Flask 提供 JSON REST API（`/api/tasks`、`/api/tasks/<id>/stream` SSE 等），Vue 3 + Pinia 维护前端状态机（`idle → sending → streaming → completed/failed/waiting`），不依赖服务端渲染 HTML 片段。协作者可见 **`docs/2026-05-04-cockpit-ui-progress.md`**。
...
```bash
rtk git add README.md .gitignore AGENTS.md bridge docs examples GUI requirements-dev.txt scripts skills tests 落地方案.md
```
