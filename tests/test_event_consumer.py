...
def test_consumer_ignores_hidden_state_json_files(tmp_path: Path) -> None:
    event_dir = tmp_path / "events"
    event_dir.mkdir()
    (event_dir / ".consumer-state.json").write_text(
        json.dumps({"processed_message_ids": [], "skipped_message_ids": [], "failed_message_ids": []}),
        encoding="utf-8",
    )
    event_file = event_dir / "im.message.receive_v1_a.json"
    write_event(event_file, "om_123")
    ...
    consumer = EventConsumer(
        EventConsumerConfig(event_dir=event_dir, tasks_root=tmp_path / "tasks", state_path=event_dir / ".consumer-state.json"),
        handler=lambda path: processed.append(path),
    )
...
def test_consumer_moves_malformed_events_to_failed_dir_without_crashing(tmp_path: Path) -> None:
    event_dir = tmp_path / "events"
    event_dir.mkdir()
    event_file = event_dir / "bad.json"
    event_file.write_text(json.dumps({"type": "im.message.receive_v1", "chat_id": "oc_123"}), encoding="utf-8")
    processed: list[Path] = []
    ...
    assert failed_file.exists()
    state = json.loads((tmp_path / "state.json").read_text(encoding="utf-8"))
    assert state["failed_message_ids"] == ["bad.json"]
